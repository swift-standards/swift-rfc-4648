...
struct BinaryIntegerEncodingTests {
    ...
    func testBase64RoundTrip() {
        let values: [UInt32] = [0, 1, 255, 256, 65535, 123456, UInt32.max]
    ...
    func testBase64URLEncoding() {
        ...
        #expect(String(base64URL: UInt32(0)) == "AAAAAA")  // No padding by default
        #expect(String(base64URL: UInt32(123456)) == "AAHiQA")
    }

    @Test("Base64URL: Padding control")
    func testBase64URLPadding() {
        let value = UInt32(123456)
        #expect(String(base64URL: value, padding: false) == "AAHiQA")
        #expect(String(base64URL: value, padding: true) == "AAHiQA==")
    }

    @Test("Base64URL: Different from Base64")
    func testBase64URLDifference() {
        ...
        let base64URL = String(base64URL: value, padding: false)
    ...
    func testBase32Encoding() {
        ...
        #expect(String(base32: UInt32(123456)) == "AAA6EQA=")
    }

    @Test("Base32: Padding control")
    func testBase32Padding() {
        let value = UInt32(123456)
    ...
    func testBase32HexEncoding() {
        // UInt32(0) = [0x00, 0x00, 0x00, 0x00] in big-endian
        #expect(String(base32Hex: UInt32(0)) == "0000000=")
        // UInt32(123456) = [0x00, 0x01, 0xE2, 0x40] in big-endian
        #expect(String(base32Hex: UInt32(123456)) == "000U4G0=")
    }

    @Test("Base32-HEX: Padding control")
    func testBase32HexPadding() {
        let value = UInt32(123456)
        // UInt32(123456) = [0x00, 0x01, 0xE2, 0x40] in big-endian → "000U4G0="
        #expect(String(base32Hex: value, padding: true) == "000U4G0=")
        #expect(String(base32Hex: value, padding: false) == "000U4G0")
    }

    @Test("Base32-HEX: Different from Base32")
    func testBase32HexDifference() {
        // Base32-HEX uses 0-9, A-V (Extended Hex Alphabet)
        // Base32 uses A-Z, 2-7
        let value = UInt32(123456)

        let base32 = String(base32: value, padding: false)
        let base32Hex = String(base32Hex: value, padding: false)
    ...
    func testBigEndianConsistency() {
        ...
        let base64FromBytes = String(base64Encoding: expectedBytes)
    ...
    func testZeroValue() {
        #expect(String(hex: UInt8(0)) == "0x00")
        #expect(String(base64: UInt8(0)) == "AA==")
        #expect(String(base64URL: UInt8(0)) == "AA")
        #expect(String(base32: UInt8(0)) == "AA======")
        #expect(String(base32Hex: UInt8(0)) == "00======")
    }
    ...
    func testMaximumValues() {
        ...
        _ = String(base32Hex: UInt8.max)
        _ = String(base32Hex: UInt16.max)
    }

    // MARK: - Type Flexibility

    @Test("All BinaryInteger types supported")
    func testVariousIntegerTypes() {
        // UInt family
        ...
        _ = String(base64URL: Int32(42))
        _ = String(base32: UInt16(42))
        _ = String(base32Hex: Int64(42))
    }
}
